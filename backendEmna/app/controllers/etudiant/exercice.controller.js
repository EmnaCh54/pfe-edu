...
exports.findOne = async (req, res) => {
  try {
    const exercice = await Exercice.findById(req.params.exerciceId).populate(
      "contenu_id"
    );
    ...
    if (!exercice) {
      return res
        .status(404)
        .send({
          message: `exercice non trouvé avec l'ID ${req.params.exerciceId}`,
        });
    }
...
// Télécharger un exercice par ID
exports.downloadExerciceById = async (req, res) => {
  try {
    const exercice = await Exercice.findById(req.params.coursId);
    if (!exercice) {
      return res
        .status(404)
        .send({
          message: `Exercice not found with ID ${req.params.exerciceId}`,
        });
    }
  } catch (error) {
    console.error(error);
